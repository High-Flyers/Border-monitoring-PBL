...
while True:
    ...
    cv2.setWindowProperty('frame', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)  # Ustawianie trybu pełnoekranowego
    ...
    if key == ord(' '):
        ...
        if not save:
            ...
            writer = cv2.VideoWriter(os.path.join(photodir, f'video{j}.mp4'), cv2.VideoWriter_fourcc(*"mp4v"), 9,
                                     (width, height))
            j += 1
        else:
            print("Saved video ", j)
